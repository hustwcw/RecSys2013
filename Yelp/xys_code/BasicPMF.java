...
public class BasicPMF {
		...
		private static final double Lamda = 0.1; //lamda_u = lamda_v 
		private static final double Sparsity = 0.2; //matrix density
		private static final int D = 10;  //latent factor dim
		private static final double Lrate = 0.0012;  //learning rate
		
		private static final int MaxIterNum = 30;  //下降次数
		private static final double iterThreshold = 0.005;
		
		private static final double IterNum = 5;
		...
		private void loadMatrix() throws IOException {
			...
			SparseRTMatrix = VectorUtil.loadMatrix(SparseMatrixFile, UserSize, ServiceSize);
